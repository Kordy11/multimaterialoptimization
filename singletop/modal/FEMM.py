...
def FEM(nelx, nely, x, pK, pM, Emin, Emax, rho, optf):
	"""
	...
	'pK' and 'pM' is a penalization factor stiffness and mass used for SIMP model
	'Emin' is elastic modulus of void
	'Emax' is elastic modulus of material
	'rho' is density
	...
	# ----------ELEMENT MASS MATRIX----------
	ME = (rho/36)*np.array([[4, 0, 2, 0, 1, 0, 2, 0],
							[0, 4, 0, 2, 0, 1, 0, 2],
							[2, 0, 4, 0, 2, 0, 1, 0],
							[0, 2, 0, 4, 0, 2, 0, 1],
							[1, 0, 2, 0, 4, 0, 2, 0],
	...
	sK = ((KE.flatten()[np.newaxis]).T*(Emin + x**pK*(Emax - Emin))).flatten(order='F')
	K = coo_matrix((sK, (iKM, jKM)), shape=(ndof, ndof)).tocsc()
	mK = ((ME.flatten()[np.newaxis]).T*(Emin + x**pM*(Emax - Emin))).flatten(order='F')
